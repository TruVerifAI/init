...
function mergeJson(file, empty, mutate, note) {
  try {
    ...
    if (fs.existsSync(file)) {
      // A zero-byte / whitespace-only file counts as EMPTY, not corrupt:
      // some hosts (Antigravity) create their config as a zero-byte stub,
      // and an empty file has no user content a merge could lose, so
      // initializing it is strictly safe for every writer sharing this
      // helper. Claude-owned files (.credentials.json, settings.json) have
      // their own writers and their own never-create rules — not this one.
      // Strip a leading BOM before BOTH checks: editors emit BOM-prefixed
      // valid JSON, which JSON.parse refuses.
    ...
    if (existing === null) {
      return {
        installed: false,
        // error-grade so init renders ✗ — doctor shows the same condition as
        // an error, and a hard refusal must not read as an ignorable note.
        severity: "error",
...
function macKeychainHasClaudeCreds(runner) {
  ...
  try {
    ...
    // Exit 44 (errSecItemNotFound) is `security` ANSWERING "no such item" --
    // a real "absent". Any OTHER nonzero (locked keychain, permission denied,
    // daemon error) is an outage, and must not read as absent (audit
    // mcp_1f1f04ba F-003 -- the comment here always said 44; the code
    // collapsed every nonzero).
...
/** Read the Claude Code credentials blob from the login Keychain.
 *  Returns {state:"ok", blob} | {state:"absent"} | {state:"unavailable"}.
 *  Tri-state for the same reason as macKeychainHasClaudeCreds: a timeout or
 *  spawn failure must never read as "absent". Strips exactly ONE trailing
 *  newline (what security -w appends). The blob is held in memory only --
 *  never logged, never returned in notes. */
function macKeychainReadBlob(runner) {
  if (process.platform !== "darwin" && !runner) return { state: "absent" };
  try {
    const run = runner || ((cmd, args, opts) =>
      require("child_process").spawnSync(cmd, args, opts));
    ...
    if (r.error) return { state: "unavailable" };
    // Exit 44 = errSecItemNotFound -- the only nonzero that MEANS absent.
    // Locked keychain / permission denied / daemon errors are all nonzero too,
    // and collapsing them into "absent" turns an outage into a false
    // "missing" (audit mcp_1f1f04ba F-003).
    if (r.status === 44) return { state: "absent" };
    if (r.status !== 0) return { state: "unavailable" };
    let blob = String(r.stdout == null ? "" : r.stdout);
    if (blob.endsWith("\n")) blob = blob.slice(0, -1);
    if (!blob) return { state: "unavailable" }; // empty secret = wrong; never treat as data
    return { state: "ok", blob: blob };
  } catch (e) {
    return { state: "unavailable" };
  }
}

/** Doctor's read surface (item 4b): does the Keychain blob carry the plugin
 *  api_token? "present" | "missing" | "unavailable". Existence check only --
 *  the value never leaves this function. "unavailable" covers every failure
 *  (different storage scheme, timeout, non-JSON blob): not knowing must render
 *  as "cannot verify", never as "missing", or the F-2 false negative returns
 *  in a new form. */
function macKeychainPluginTokenState(runner) {
  const r = macKeychainReadBlob(runner);
  if (r.state !== "ok") return "unavailable";
  try {
    const d = JSON.parse(r.blob);
    const leaf = d && d.pluginSecrets && d.pluginSecrets[CLAUDE_PLUGIN_ID]
      && d.pluginSecrets[CLAUDE_PLUGIN_ID].api_token;
    return leaf ? "present" : "missing";
  } catch (e) {
    return "unavailable";
  }
}

/** BEST-EFFORT write of the plugin api_token into the Keychain blob (item 4c).
 *  Every exit except "set"/"already" means: fall back to the manual message.
 *  Proven mechanics (probe-results.md K1-K5, macOS 12): the blob is compact
 *  JSON {claudeAiOauth, pluginSecrets}; add-generic-password -U updates in
 *  place and preserves the ACL; the JSON round-trip is format-preserving.
 *  Claude Code's own sidecar reads this item via /usr/bin/security, so we
 *  write with the same principal it reads with.
 *
 *  Guards (all fail toward the manual fallback, never toward a blind write):
 *  - UPDATE only, never create: a security-created item could carry a
 *    partition list other Claude Code builds cannot read;
 *  - the blob must parse AND contain claudeAiOauth before anything is written;
 *  - compare-and-swap: re-read immediately before writing and abort if the
 *    blob moved (Claude Code rewrites it on OAuth refresh -- racing it would
 *    clobber fresh tokens and log the user out);
 *  - write back the PARSED OBJECT wholesale (unknown keys survive), compact
 *    JSON.stringify -- the same format Claude Code stores;
 *  - -X (hex) so the value needs no shell quoting; argv exposure adds no new
 *    principal on this platform (the item is already readable by any local
 *    process via the security tool -- Silverfort 2026-07);
 *  - post-write verification re-reads and checks the leaf. */
function macKeychainSetPluginToken(key, runner) {
  try {
    const run = runner || ((cmd, args, opts) =>
      require("child_process").spawnSync(cmd, args, opts));
    const meta = run("security",
      ["find-generic-password", "-s", KEYCHAIN_SERVICE],
      { encoding: "utf8", timeout: 10000 });
    if (meta.error || meta.status !== 0) return { state: "failed", why: "item metadata unreadable" };
    // The `security` textual output is not a stable API; this exact shape
    // ("acct"<blob>="...") is verified against macOS 12 (probe-results.md) and
    // matches the format Claude Code's own sidecar parses. An account name
    // containing an escaped quote would truncate here -- acceptable: the write
    // then fails toward the manual fallback, never toward a wrong-account write.
    const m = /"acct"<blob>="([^"]*)"/.exec(String(meta.stdout || ""));
    if (!m) return { state: "failed", why: "account attribute not found" };
    const acct = m[1];

    const r1 = macKeychainReadBlob(runner);
    if (r1.state !== "ok") return { state: "failed", why: "blob read " + r1.state };
    let d;
    try {
      d = JSON.parse(r1.blob);
    } catch (e) {
      return { state: "failed", why: "blob is not JSON" };
    }
    if (typeof d !== "object" || d === null || Array.isArray(d) || !d.claudeAiOauth) {
      return { state: "failed", why: "blob shape unexpected (no claudeAiOauth)" };
    }
    const existing = d.pluginSecrets && d.pluginSecrets[CLAUDE_PLUGIN_ID]
      && d.pluginSecrets[CLAUDE_PLUGIN_ID].api_token;
    if (existing === key) return { state: "already" };

    d.pluginSecrets = (typeof d.pluginSecrets === "object" && d.pluginSecrets !== null && !Array.isArray(d.pluginSecrets)) ? d.pluginSecrets : {};
    d.pluginSecrets[CLAUDE_PLUGIN_ID] = (typeof d.pluginSecrets[CLAUDE_PLUGIN_ID] === "object" && d.pluginSecrets[CLAUDE_PLUGIN_ID] !== null) ? d.pluginSecrets[CLAUDE_PLUGIN_ID] : {};
    d.pluginSecrets[CLAUDE_PLUGIN_ID].api_token = key;
    const out = JSON.stringify(d);
    if (!out || out === "{}") return { state: "failed", why: "refusing empty payload" };

    const r2 = macKeychainReadBlob(runner);
    if (r2.state !== "ok" || r2.blob !== r1.blob) {
      return { state: "raced", why: "blob changed between read and write" };
    }
    // RESIDUAL RACE, documented (audit mcp_1f1f04ba F-001, disposition agreed
    // with the owner 2026-08-18): a refresh landing between this re-read and
    // the write below overwrites the refreshed OAuth tokens with the ones we
    // just read -- and is invisible to any post-write check by construction,
    // because the write destroys the evidence (the blob then equals exactly
    // what we expected). Comparing claudeAiOauth after the fact was
    // considered and REJECTED: it cannot see this case, and it false-fails
    // the healthy one (Claude refreshing AFTER our successful write). The
    // window is the milliseconds between these two adjacent calls; the blast
    // radius is a Claude re-login (probe-results.md, accepted mitigation).
    const hex = Buffer.from(out, "utf8").toString("hex");
    const w = run("security",
      ["add-generic-password", "-U", "-a", acct, "-s", KEYCHAIN_SERVICE, "-X", hex],
      { encoding: "utf8", timeout: 10000 });
    if (w.error || w.status !== 0) return { state: "failed", why: "keychain write failed" };

    const r3 = macKeychainReadBlob(runner);
    if (r3.state !== "ok") return { state: "failed", why: "post-write verify read failed" };
    try {
      const v = JSON.parse(r3.blob);
      if (v && v.claudeAiOauth && v.pluginSecrets && v.pluginSecrets[CLAUDE_PLUGIN_ID]
          && v.pluginSecrets[CLAUDE_PLUGIN_ID].api_token === key) {
        return { state: "set" };
      }
    } catch (e) { /* falls through */ }
    return { state: "failed", why: "post-write verification did not match" };
  } catch (e) {
    return { state: "failed", why: String(e && e.message || e).slice(0, 80) };
  }
}

function writeClaudeCreds(key, keychainProbe, keychainWriter) {
  ...
  if (!fs.existsSync(file)) {
    ...
    const kc = probe();
    if (kc === "found") {
      // Claude Code IS here, Keychain-backed. BEST-EFFORT (item 4c): the
      // token's store is pluginSecrets[<plugin>@<marketplace>].api_token
      // inside Claude's one credentials blob (probe-results.md K1), and a
      // guarded read-modify-write of it is proven safe (K2-K5). Try that;
      // ANY anomaly falls back to the honest manual message -- never a blind
      // write, never creating the item.
      const writeToken = keychainWriter || macKeychainSetPluginToken;
      const kw = writeToken(key);
      if (kw.state === "set") {
        return { installed: true, keychain: true, notes: [
          "plugin api_token set (macOS Keychain) -- verified after write",
          "run /reload-plugins in open Claude Code sessions",
        ] };
      }
      if (kw.state === "already") {
        return { installed: true, keychain: true, notes: [
          "plugin api_token already set (macOS Keychain)",
        ] };
      }
      return {
        installed: false,
        ...
        notes: [
          ...
          "Claude Code stores credentials in the macOS Keychain; setting the api_token",
          "automatically did not work (" + (kw.why || kw.state) + ").",
          "Set it once: get your API key from truverif.ai (it is also in ~/.truverifai/config.json),",
          "then:  /plugin -> AI Panel Review -> Configure -> api_token",
...
const MARKETPLACE_URL = "https://github.com/TruVerifAI/claude-plugins.git";

/** Enable marketplace auto-update for the truverifai marketplace (item 4a).
 *  Auto-update is OFF by default for third-party marketplaces, so an installed
 *  Claude user receives NO plugin updates (security fixes included) unless
 *  they find the toggle. Probe B (probe-results.md) established the toggle is
 *  a plain leaf: extraKnownMarketplaces.truverifai.autoUpdate in
 *  ~/.claude/settings.json (mirrored by Claude Code into its derived
 *  known_marketplaces.json -- we write ONLY the settings surface). Absent
 *  means off. An EXPLICIT false is a user choice and is respected -- only an
 *  absent key is upgraded to true. Same discipline as the other settings.json
 *  writers: never create the file, shape-validate, backup, additive merge,
 *  post-validate, atomic rename. */
function writeClaudeMarketplaceAutoUpdate() {
  const file = path.join(homeDir(), ".claude", "settings.json");
  const manual = "manual path: /plugin -> Marketplaces -> truverifai -> Enable auto-update";
  if (!fs.existsSync(file)) {
  ...
  try {
    ...
    try {
      ...
      return { installed: false, severity: "error", notes: ["~/.claude/settings.json is not valid JSON -- not touching it; " + manual] };
    }
    if (typeof d !== "object" || d === null || Array.isArray(d)) {
      return { installed: false, severity: "error", notes: ["~/.claude/settings.json has an unexpected shape -- not touching it; " + manual] };
    }
    const ekm = d.extraKnownMarketplaces;
    if (ekm !== undefined && (typeof ekm !== "object" || ekm === null || Array.isArray(ekm))) {
      return { installed: false, severity: "error", notes: ["settings.json extraKnownMarketplaces has an unexpected shape -- not touching it; " + manual] };
    }
    const entry = ekm && ekm.truverifai;
    if (entry !== undefined && (typeof entry !== "object" || entry === null || Array.isArray(entry))) {
      return { installed: false, severity: "error", notes: ["settings.json extraKnownMarketplaces.truverifai has an unexpected shape -- not touching it; " + manual] };
    }
    if (entry && entry.autoUpdate === true) {
      return { installed: true, notes: ["marketplace auto-update already enabled"] };
    }
    if (entry && entry.autoUpdate === false) {
      // The user explicitly turned it off -- their call stands.
      return { installed: false, notes: ["marketplace auto-update left OFF (explicitly disabled in your settings); plugin updates arrive on `npx @truverifai/init@latest` or " + manual] };
    }
    const priorTopKeys = Object.keys(d).length;
    // Snapshot BEFORE mutating: `entry` is the same object we mutate below,
    // so a post-hoc Object.keys(entry) would already include the new leaf.
    const priorEntryNames = entry ? Object.keys(entry) : [];
    const priorSource = entry ? JSON.stringify(entry.source) : null;
    fs.copyFileSync(file, backup);
    d.extraKnownMarketplaces = ekm || {};
    // Same shape the /plugin UI writes (probe B). Only created when absent;
    // an existing entry keeps every key it had and gains only autoUpdate.
    d.extraKnownMarketplaces.truverifai = entry || { source: { source: "git", url: MARKETPLACE_URL } };
    d.extraKnownMarketplaces.truverifai.autoUpdate = true;
    const post = d.extraKnownMarketplaces.truverifai;
    // Semantic invariants, not key-count arithmetic (audit mcp_1f1f04ba
    // F-005): the leaf is set, every pre-existing entry key survives, and a
    // pre-existing source is byte-identical.
    const bad =
      Object.keys(d).length < priorTopKeys ||
      post.autoUpdate !== true ||
      priorEntryNames.some((k) => !(k in post)) ||
      (priorSource !== null && JSON.stringify(post.source) !== priorSource);
    if (bad) {
      fs.copyFileSync(backup, file);
      return { installed: false, severity: "error", notes: ["post-merge validation failed -- restored backup; " + manual] };
    }
    const tmp = file + ".tvai-tmp";
    fs.writeFileSync(tmp, JSON.stringify(d, null, 2) + "\n", { encoding: "utf8" });
    fs.renameSync(tmp, file);
    return { installed: true, notes: [
      "marketplace auto-update enabled -- plugin updates now install at session start",
      "(disable any time: /plugin -> Marketplaces -> truverifai)",
    ] };
  } catch (e) {
    try {
      if (fs.existsSync(backup)) fs.copyFileSync(backup, file);
    } catch (e2) { /* best effort restore */ }
    return { installed: false, severity: "error", notes: ["settings write failed (" + String(e.message).slice(0, 80) + ") -- restored backup; " + manual] };
  }
}
...
module.exports = {
  ...
  writeClaudeMarketplaceAutoUpdate,
  macKeychainPluginTokenState,
  macKeychainSetPluginToken,
  macKeychainReadBlob,
