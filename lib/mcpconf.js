...
function writeClaudeCreds(key) {
  ...
  if (!fs.existsSync(file)) {
    // Never CREATE this file — Claude Code owns its shape. (No file = the user
    // has not run Claude Code; the plugin install will surface that first.)
    return { installed: false, notes: ["~/.claude/.credentials.json not found — " + manual] };
  }
