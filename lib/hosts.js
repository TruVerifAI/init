...
/** The repo ROOT for `cwd`, or null when not in a work tree.
 *
 *  `detect.in_git_repo` only checks `cwd/.git`, so running init from a
 *  SUBDIRECTORY reported "not a git repo" and skipped the gate — the exact
 *  incident that started X11, reproduced by the fix for it (audit F-003).
 *  Asking git is also correct for worktrees and submodules, where `.git` is a
 *  FILE rather than a directory (F-008). Falls back to the naive check if git
 *  is unavailable, so we never do worse than before. */
function gitRepoRoot(cwd) {
  ...
  return fs.existsSync(path.join(cwd, ".git")) ? cwd : null;
}
