...
function gateStateRows(ok, warn) {
  ...
  if (!s.split) {
    rows.push(line("all deliveries", s.universal, s.universalSource));
  } else {
    ...
    rows.push(line("every other delivery (git pre-commit hook, Codex, Cursor, "
      + "Copilot, VS Code, Gemini, Antigravity)", s.universal, s.universalSource));
...
function claudeInstalledHookRow(ok, bad) {
  const { root, candidates } = activeClaudeBundle();
  if (!candidates.length) return null;
  if (!root) {
    return bad("[claude] cannot determine the active plugin bundle — "
      + candidates.length + " non-orphaned versions present ("
      + candidates.map((d) => path.basename(d)).join(", ")
      + "). Hook commands NOT verified. Run /plugin update, then re-run doctor.");
  }
  let hk;
  try {
    hk = JSON.parse(fs.readFileSync(path.join(root, "hooks.json"), "utf8"));
  } catch (e) {
    return bad("[claude] installed hooks.json is missing or invalid JSON: " + root);
  }
  const problems = [];
  for (const groups of Object.values((hk && hk.hooks) || {})) {
    for (const grp of groups || []) {
      for (const entry of grp.hooks || []) {
        ...
        // A BARE ${CLAUDE_PLUGIN_ROOT}/....sh command relies on the POSIX exec
        // bit, which a Windows-authored repo cannot record — it dies
        // "Permission denied" on macOS/Linux and the gate no-ops SILENTLY.
        // Flag it on every OS: shipping it is wrong even where it happens to work.
...
async function run(argv) {
  ...
  if (det.claude) {
    ...
    const hookRow = claudeInstalledHookRow(ok, bad);
...
module.exports = {
  ...
  nodeExe, claudeInstalledHookRow, activeClaudeBundle,
