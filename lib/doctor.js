...
async function run(argv) {
  ...
  if (key) {
    ...
    if (det.claude) {
      ...
      if (kc === "found") {
        rows.push(bad("[claude] tools NOT connected — Claude Code stores credentials "
          + "in the macOS Keychain, so the api_token cannot be pre-populated "
          + "automatically. Set it once: /plugin -> panel-review -> Configure -> "
...
module.exports = {
  run, syntheticFire, pyExe, recordedPython, gateStateRows,
