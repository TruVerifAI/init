...
/** The interpreter record, shared with the gates themselves.
 *
 *  Loaded from the VENDORED resolver rather than reimplemented: doctor and the
 *  launcher must agree about where the record lives and how home is resolved,
 *  and the only way to guarantee that is one module. */
function resolverModule() {
  try {
    return require("../vendor/gates/resolve_python.js");
  } catch (e) {
    return null;
  }
}

/** Which interpreter will the gates actually use?
 *
 *  doctor used to answer this by SEARCHING — its own probe, with its own rules,
 *  in a process that had already spawned git and a plugin install. The launcher
 *  searched too, with DIFFERENT rules (doctor demanded Python 3, the launcher
 *  accepted anything that started). Two searchers, two answers, and the one
 *  that reported was not the one that ran. That is precisely how the Windows
 *  launcher abort stayed invisible while this tool printed green (X13a).
 *
 *  So the search is gone from here (roadmap 1.5). There is exactly one
 *  searcher now — `resolve_python.js`, at install time — and doctor reports
 *  what it recorded. Two searchers cannot drift apart if there is only one.
 *
 *  Returns {python, reason}: the absolute path the gates will launch, or null
 *  plus the recorded reason why there isn't one. */
function recordedPython() {
  const R = resolverModule();
  if (!R) return { python: null, reason: "the vendored resolver is missing from this package" };
  const rec = R.readRecord();
  if (rec && rec.python) {
    try {
      if (fs.statSync(rec.python).isFile()) return { python: rec.python, reason: null };
    } catch (e) {
      return { python: null, reason: "the recorded interpreter no longer exists: " + rec.python };
    }
    return { python: null, reason: "the recorded interpreter is not a file: " + rec.python };
  }
  let detail = "";
  try {
    const r = JSON.parse(fs.readFileSync(R.reasonFile(), "utf8"));
    detail = [r.reason, r.detail].filter(Boolean).join(" — ");
  } catch (e) {
    /* no reason recorded */
  }
  return { python: null, reason: detail || "no interpreter has been recorded yet" };
}

/** Back-compat shim for callers that just want the interpreter or null. */
function pyExe() {
  return recordedPython().python;
}
...
function syntheticFire(platform, apiKeyVal) {
  const py = pyExe();
  if (!py) return { armed: false, detail: "no recorded python — " + recordedPython().reason };
...
function nestedRepoCheck(apiKeyVal) {
  const py = pyExe();
  if (!py) return { ok: false, detail: "nested-repo check skipped: no recorded python" };
...
function gitPrecommitRow(ok, bad, warn, cwd) {
  ...
  if (text.includes("truverifai pre-commit commit gate")) {
    ...
    // Item 12: two shapes exist in the wild. The current hook runs the shared
    // launcher (run_gate.js); the pre-0.19.38 shape did its own interpreter
    // search with silent fail-open — functional, but missing every round-2
    // protection, so it gets a warn that names the upgrade.
    // Shape note (audit mcp_9e0da2ce F-001): this match depends on the hook
    // template's double-quoted path form. If the template's quoting ever
    // changes, the miss falls through to the generic could-not-read WARN —
    // safe (never a false green), but update this matcher with the template.
    const lm = text.match(/"([^"]*run_gate\.js)"/);
    if (lm) {
      if (!fs.existsSync(lm[1])) {
        return bad("[git] the pre-commit gate points at a missing launcher ("
          + lm[1] + ") — it prints a warning and allows, so commits here are "
          + "NOT gated. Fix: npx @truverifai/init");
      }
      const gateFile = path.join(path.dirname(lm[1]), "audit_gate.py");
      if (!fs.existsSync(gateFile)) {
        return bad("[git] the pre-commit gate's launcher exists but the gate "
          + "script is missing (" + gateFile + ") — commits here are NOT "
          + "gated. Fix: npx @truverifai/init");
      }
    ...
    if (m) {
      return warn("[git] this repo's pre-commit gate is the pre-0.19.38 shape "
        + "(its own interpreter search, silent fail-open, no self-heal) — it "
        + "works, but re-run `npx @truverifai/init hook` here to upgrade it to "
        + "the shared launcher.");
    }
...
async function run(argv) {
  ...
  // These two rows are measured over NODE's HTTPS client — and are LABELLED as
  // such (roadmap 5.9). On the 2026-08-17 macOS round both stayed green through
  // a total gate outage: the gates make their calls from PYTHON, that machine's
  // Python had no usable trust store, and node's HTTPS was fine. Literally true
  // rows, practically misleading — because nothing said which route they
  // measured. The row that answers "can the GATES reach us?" is the
  // [gate endpoint] row below, which runs the real gate code under the recorded
  // interpreter.
  rows.push((await api.healthCheck(base))
    ? ok("backend reachable: " + base + " (node client — the [gate endpoint] row below covers the gates' own route)")
    : bad("backend UNREACHABLE via the node client: " + base));
  ...
  if (!key) {
    ...
    keyValid = kc.valid;
    rows.push(kc.valid ? ok("key valid (source: " + keySource() + "; node client)") : bad("key INVALID (HTTP " + kc.status + ", source: " + keySource() + ") — run `npx @truverifai/init login` for a fresh key, then re-run `npx @truverifai/init` so every config picks it up"));
  }

  // 1.5/1.10: report the RECORDED interpreter by absolute path — "python
  // resolved: python3" never said WHICH python3, and a machine can easily have
  // three with different trust stores. On failure, print the reason the gates
  // themselves recorded rather than a guess.
  // Say exactly what this row checked, and no more. It reads the record and
  // confirms the file is there — it does NOT prove the interpreter can run a
  // gate. That proof is the [launcher/*] and per-platform fire rows below,
  // which execute the real thing. Claiming more here would be a false green of
  // precisely the species this tool has now shipped three times.
  const rp = recordedPython();
  rows.push(rp.python
    ? ok("python recorded: " + rp.python + " (present; the fire rows below prove it runs)")
    : bad("NO PYTHON RECORDED — the gates cannot run. " + rp.reason
  ...
  // THE GATES' OWN ROUTE (roadmap 5.9): gate_selfcheck.py, run under the
  // recorded interpreter — the same code path, HTTP stack and trust store a
  // real gate uses. This is the row that goes red when Python's TLS is broken
  // while node's works (the exact split that made the two rows above green
  // through a total outage on the macOS round). Skipped without a key: the
  // check needs one, and the per-platform section below explains that case.
  if (key && rp.python) {
    const gsc = hostsMod.runSelfCheck(rp.python, key);
    rows.push((gsc.installed ? ok : bad)(
      "[gate endpoint] " + gsc.notes.join(" ")
      + (gsc.installed ? " (checked over the gates' own route: recorded python + its TLS)" : "")));
  } else if (key) {
    rows.push(bad("[gate endpoint] not checked — no recorded interpreter (see the python row above)"));
  } else {
    // A silently ABSENT row is its own false-green class (audit mcp_4a363094
    // F-002): an operator who does not know the row exists reads "no red" as
    // clean. Skipping must be visible and visually distinct from a pass.
    rows.push(warn("[gate endpoint] SKIPPED — no API key, the gates' own route is untested (run tvai login)"));
  }
  ...
  if (key) {
    ...
    const toolSpots = [];
    // Claude Code is special-cased (roadmap 2.3): on macOS its credentials
    // live in the Keychain and ~/.claude/.credentials.json never exists, so
    // the generic "no file -> re-run tvai init" remedy is UNSATISFIABLE there —
    // it survived five init runs including a full reinstall on the Mac round.
    // When the Keychain holds Claude's credentials, say the true cause and the
    // instruction that works instead.
    if (det.claude) {
      const credsFile = path.join(HOME, ".claude", ".credentials.json");
      const kc = fs.existsSync(credsFile) ? null : mcpconf.macKeychainHasClaudeCreds();
      if (kc === "found") {
        // Item 4b: the blob is a real read surface -- check the token leaf's
        // EXISTENCE (never the value). "unavailable" renders as cannot-verify,
        // never as missing (F-2: the old row was a PERMANENT false negative
        // here -- it said NOT connected while ping authenticated fine).
        const tok = mcpconf.macKeychainPluginTokenState();
        if (tok === "present") {
          rows.push(ok("[claude] tools connected (api_token present in the macOS Keychain)"));
        } else if (tok === "missing") {
          rows.push(bad("[claude] tools NOT connected — no api_token in the macOS "
            + "Keychain. Set it once: get your API key from truverif.ai (also in "
            + "~/.truverifai/config.json) -> /plugin -> AI Panel Review -> Configure -> "
            + "api_token, then /reload-plugins"));
        } else {
          rows.push(warn("[claude] tools — cannot verify from here (Keychain read "
            + "did not complete); run the `ping` tool in a Claude session to confirm. "
            + "If not connected: /plugin -> AI Panel Review -> Configure -> api_token, "
            + "then /reload-plugins"));
        }
      } else if (kc === "unavailable") {
        // Not knowing is an answer of its own (audit mcp_c06b6c66 F-002):
        // routing this through the generic not-installed row would prescribe
        // the unsatisfiable rerun remedy on exactly the machines it never
        // works on. (A test bans that phrase from this whole block — which is
        // why this comment paraphrases it.)
        rows.push(warn("[claude] tools state UNKNOWN — no credentials file, and "
          + "the macOS Keychain check could not complete. If you use Claude Code "
          + "here: /plugin -> panel-review -> Configure -> api_token, then /reload-plugins"));
      } else {
        toolSpots.push(["claude", credsFile, "panel-review@truverifai"]);
      }
    }
    // Item 4a read surface: warn when marketplace auto-update is off (absent
    // means off -- probe B). Settings.json is the user-owned surface.
    if (det.claude) {
      try {
        const sj = JSON.parse(fs.readFileSync(path.join(HOME, ".claude", "settings.json"), "utf8"));
        const au = sj && sj.extraKnownMarketplaces && sj.extraKnownMarketplaces.truverifai
          && sj.extraKnownMarketplaces.truverifai.autoUpdate;
        rows.push(au === true
          ? ok("[claude] marketplace auto-update ON (plugin updates install at session start)")
          : warn("[claude] marketplace auto-update OFF — plugin updates will NOT arrive on their own; re-run `npx @truverifai/init` to enable it (or /plugin -> Marketplaces -> truverifai)"));
      } catch (e) { /* no settings file readable -- the tools rows already cover that state */ }
    }
    ...
    if (det.antigravity) toolSpots.push(["antigravity", path.join(HOME, ".gemini", "config", "mcp_config.json"), mcpconf.SERVER]);
    if (det.vscode) {
  ...
  return exitCodeForRows(rows, argv.includes("--strict"));
}

/** Exit-code contract (round-3 item 9): "the gates are healthy" means exit 0.
 *  A ✗ on a review-TOOLS row is real and stays visible, but it does not mean
 *  the gates are broken — on a Keychain-backed Mac the tools ✗ was PERMANENT,
 *  so every scripted `doctor` call read as failure while every gate row was
 *  green (round-3 finding). Default: only gate-delivery ✗ rows drive a
 *  nonzero exit; `--strict` restores any-✗ = 1. FAIL-CLOSED for the unknown:
 *  a ✗ row this list doesn't recognize counts as gate-critical. */
function exitCodeForRows(rows, strict) {
  // MAINTENANCE CONTRACT (audit mcp_c6c48301 F-001): the nonCritical
  // exemption matches RENDERED row text. If you change any tools-row
  // wording, update this list — the exit-contract tests pin both sides, and
  // the fail-closed default means a missed update fails toward exit 1 (a
  // false failure), never toward a hidden gate breakage.
  const isBad = (r) => r.startsWith("  ✗");
  if (strict) return rows.some(isBad) ? 1 : 0;
  const nonCritical = (r) =>
    r.includes("tools NOT connected") || r.includes("tools state UNKNOWN")
    || r.includes("tools — cannot verify");
  return rows.some((r) => isBad(r) && !nonCritical(r)) ? 1 : 0;
}
...
module.exports = {
  run, syntheticFire, pyExe, recordedPython, gateStateRows, exitCodeForRows,
