...
function renderStatus(s) {
  ...
  lines.push("    2. Claude /plugin toggle       "
    + (s.claude === null
        ? "not set (setting removed from the panel — this level is normally empty now)"
        : String(s.claude) + "   LEFTOVER from a removed setting — clear it (see below)")
    + "   (Claude Code hooks ONLY — it cannot reach other hosts)");
  if (s.claude !== null) {
    lines.push("       ^ enable_gates no longer exists in the plugin panel (X10)."
      + " This stored value predates the removal and may still be exported to"
      + " Claude Code's hooks. Clear it via /plugin -> panel-review -> Configure,"
      + " or edit ~/.claude/settings.json pluginConfigs, then /reload-plugins.");
  }
...
function run(argv) {
  ...
  if (s.claude !== null && s.claude !== want) {
    // X10 (2026-08-17): enable_gates was REMOVED from the plugin's settings
    // panel, so a stored value here is a LEFTOVER from before the removal —
    // there is no longer a panel field to change it in. Claude Code may still
    // export a stale stored value to its hooks, so it must still be reported
    // (silently ignoring it would be X8 again), but the remedy is now
    // "clear the leftover", not "go set the removed option".
    overridden.push("    ! Claude Code has a LEFTOVER enable_gates="
      + String(s.claude) + " stored from before that setting was removed from"
      + " the plugin panel; it can still outrank the file for Claude Code's OWN"
      + " hooks. Clear it: /plugin -> panel-review -> Configure -> delete the"
      + " stored enable_gates value (or edit ~/.claude/settings.json"
      + " pluginConfigs), then /reload-plugins.");
  }
